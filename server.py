...
def add_client(client_sock):
    ...
    if player_id not in players_list:
        ...
        ], 'shuffled_array': [], 'shuffle_ready': False, "shuffle_done": False}
    return player_id
...
def new_msg(client_sock):
    ...
    # SHUFFLING COMEÇA A EMBARALHAR
    elif msg["op"] == "SHUFFLING" and msg["status"]:
        shuffle_cards(client_sock, msg["id"])
        ...
        players_list[msg["id"]]["shuffled_array"] = msg["deck"]
        # print(players_list)
        if msg["id"] < len(players_list):
            shuffle_cards(players_list[msg["id"]+1]
                          ["sock"], msg["id"] + 1, msg["deck"])
